...
// pages/ssr.vue
// This page is Server-Side Rendered (SSR) — default Nuxt behavior.
// The HTML is fully built on the server and sent to the browser on each request.
// No special directive needed; SSR is Nuxt's default rendering mode.

definePageMeta({
    // SSR is the default — explicitly documented here for clarity
    ssr: true,
    title: 'SSR Dashboard Overview' // This is the text the layout will pick up
})

// This data-fetch runs on the SERVER before the HTML is sent.
// The user receives a fully-rendered page — great for SEO & first paint.
const { data: stats } = await useAsyncData('ssr-stats', () => {
    // Simulated server-side data (could be a DB call, API, etc.)
    return Promise.resolve({
        ...
        renderMode: 'Server-Side Rendered (SSR)',
    })
})
...
<template>
    <div class="min-h-screen bg-[--color-surface-sunken]">
        ...
        <!-- Render Mode Banner -->
        <div class="bg-[--color-accent] text-center py-2 text-sm font-semibold tracking-wide">
            🖥️ SSR — HTML rendered on the <strong>server</strong> before delivery · Fetched at: {{ stats?.renderTime }}
        </div>
        ...
        <main class="max-w-7xl mx-auto px-6 py-8 space-y-8">
            ...
            <div>
                ...
                <p class="lead">All metrics are fetched and rendered on the server. No client-side loading states.</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                <div class="card primary">
                    ...
                    <div class="card-body">
                        <p class="text-3xl font-black text-[--color-primary]">{{ stats?.users.toLocaleString() }}</p>
                ...
                <div class="card accent">
                    ...
                    <div class="card-body">
                        <p class="text-3xl font-black text-[--color-accent]">{{ stats?.orders.toLocaleString() }}</p>
                        ...
                        <div class="p-6">
                            <div v-if="activeTab === 'overview'">
                                ...
                                <p class="p text-[--color-text-muted]">
                                    This content was rendered on the server. When you navigated here, the server
                                    processed
                                    all data, built the full HTML, and sent it ready-to-display to your browser.
                                </p>
                                <div class="flex flex-wrap gap-3 mt-2">
                                    <span class="btn btn-xs secondary btn-outline">Vue 3</span>
                                    <span class="btn btn-xs btn-outline">Nuxt 4</span>
                                    <span class="btn btn-xs accent btn-outline">SSR Mode</span>
                                    <span class="btn btn-xs warning btn-outline">Tailwind v4</span>
                                </div>
                            </div>
                            <div v-if="activeTab === 'analytics'">
                                <h3 class="h4">Analytics</h3>
                                <p class="p text-[--color-text-muted]">Conversion rates, bounce rates, and session data
                                    would appear here, fetched server-side.</p>
                            </div>
                            <div v-if="activeTab === 'users'">
                                <h3 class="h4">User Management</h3>
                                <p class="p text-[--color-text-muted]">User list and role assignments fetched from the
                                    database on the server.</p>
                            </div>
                            <div v-if="activeTab === 'settings'">
                                <h3 class="h4">Settings</h3>
                                <p class="p text-[--color-text-muted]">Global configuration loaded securely from
                    ...
                    <div class="card-body" style="gap: 0.75rem">
                        <div v-for="item in activities" :key="item.id"
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-[--color-text-main] truncate">{{ item.user }}</p>
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                ...
                                <div class="flex flex-col gap-2">
                                    <label class="selection-group">
                                        <input type="radio" name="priority-ssr" value="high" class="radio danger" />
                                        <span class="text-sm">High</span>
                                    </label>
                                    <label class="selection-group">
                                        <input type="radio" name="priority-ssr" value="medium" class="radio warning"
                                            checked />
                                        <span class="text-sm">Medium</span>
                                    </label>
                                    <label class="selection-group">
                                        <input type="radio" name="priority-ssr" value="low" class="radio secondary" />
